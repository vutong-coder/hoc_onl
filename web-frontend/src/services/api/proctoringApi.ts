...
export const analyzeFrame = async (request: FrameAnalysisRequest): Promise<FrameAnalysisResponse> => {
  try {
    const response = await axios.post(`${API_BASE_URL}/proctoring/analyze-frame`, request);
...
export const getAllSessions = async (): Promise<ProctoringSession[]> => {
  try {
    const response = await axios.get(`${API_BASE_URL}/proctoring/sessions`);
    return response.data;
  } catch (error: any) {
...
export const getSessionById = async (sessionId: string): Promise<ProctoringSession | null> => {
  try {
    const response = await axios.get(`${API_BASE_URL}/proctoring/sessions/${sessionId}`);
    return response.data;
  } catch (error: any) {
...
export const terminateSession = async (sessionId: string): Promise<boolean> => {
  try {
    const response = await axios.post(`${API_BASE_URL}/proctoring/sessions/${sessionId}/terminate`);
...
export const reviewEvent = async (eventId: string, isReviewed: boolean): Promise<boolean> => {
  try {
    const response = await axios.patch(`${API_BASE_URL}/proctoring/events/${eventId}/review`, {
      isReviewed
    });
    return response.status === 200;
  } catch (error: any) {
...
export const getMediaByEventId = async (eventId: string): Promise<MediaCapture[]> => {
  try {
    const response = await axios.get(`${API_BASE_URL}/proctoring/events/${eventId}/media`);
    return response.data;
  } catch (error: any) {
